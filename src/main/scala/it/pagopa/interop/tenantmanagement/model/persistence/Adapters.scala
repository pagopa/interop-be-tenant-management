...
object Adapters {
  ...
  implicit class PersistentAttributesWrapper(private val p: PersistentTenantAttributes) extends AnyVal {
    def toAPI: TenantAttributes =
      TenantAttributes(certified = p.certified, declared = p.declared, verified = p.verified)
  }

  implicit class PersistentAttributesObjectWrapper(private val p: PersistentTenantAttributes.type) extends AnyVal {
    def fromSeed(attributes: TenantAttributes): PersistentTenantAttributes =
  ...
  implicit class PersistentTenantWrapper(private val p: PersistentTenant) extends AnyVal {
    def toAPI: Tenant =
      Tenant(id = p.id, certifier = p.certifier, attributes = p.attributes.toAPI)
