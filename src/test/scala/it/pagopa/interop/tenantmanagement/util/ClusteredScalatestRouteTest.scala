...
trait ClusteredScalatestRouteTest extends ScalatestRouteTest with Matchers with Informing {
  ...
  private def validRoleCheck(role: String, request: => HttpRequest, r: => Route) = request ~> r ~> check {
  ...
  private def invalidRoleCheck(role: String, request: => HttpRequest, r: => Route) = request ~> r ~> check {
